<?php
$today = date('d-m-y');
$query = "SELECT category.ctg_name,category.ctg_id,posts.* FROM posts INNER JOIN category ON category.ctg_id = posts.post_ctg   WHERE post_status = 1 ORDER BY `posts`.`post_date` DESC";
		...
		<?php while ($show = mysqli_fetch_assoc($info)) { ?>
			<div class="posts shadow bg-light">
				...
				<p class=" font-weight-light font-italic">
					<span>Author: <?php echo $show['post_author'];?> </span>
